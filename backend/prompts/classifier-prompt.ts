...
Category examples: "wrong_literal", "commented_line", "inverted_condition", "wrong_operator", "filter_logic", "security_sensitive", "ambiguous_root_cause", "module_unloadable", "empty_or_truncated_source"

SPECIAL CASES (demo/worker-logic.ts code defects):
- If the description states the file is EMPTY, WHITESPACE-ONLY, or missing essential exports: use at least L2 (category e.g. "empty_or_truncated_source" or "module_unloadable"). Never L1 — restoring a deleted or blank file is not a one-line typo fix.
- If the module FAILED TO LOAD (import/parse/compile error, "Failed to import eval module", etc.) and the text indicates structural invalidity: use at least L2 (category e.g. "module_unloadable" or "parse_error") unless the description explicitly points to a single obvious literal/typo that matches L1.
- L1 applies only when the source loads successfully and the defect is a truly localized literal, constant, or single-expression fix.
