...
// Read the REAL worker-logic.ts source (sandbox repair targets this module)
...
// Create a sandbox copy and apply the proposed fix to it
export const proposeSandboxFixTool = tool(
  async (input) => {
    try {
      // Create fresh sandbox from real file
      createSandbox();

      // Apply proposed fix to sandbox only
      ...
      // Show what the sandbox looks like now
  ...
  async (_input) => {
    try {
      ...
      const resultLines = testResult.results.map(
        (r) => `  ${r.passed ? "✅" : "❌"} ${r.name}${r.error ? ` — ${r.error}` : ""}`
      );
...
// Only called when tests pass — promotes sandbox to real file
