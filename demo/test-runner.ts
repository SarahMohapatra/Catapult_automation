...
export async function runValidationSuite(
  ...
  // Use a unique filename per invocation — Node/tsx ESM cache ignores query
  // params, so the old `?t=` approach returned stale modules.
  const evalFile = path.join(EVAL_DIR, `worker-logic.eval.${Date.now()}.ts`);

  let logic: WorkerLogicModule;
  try {
    fs.writeFileSync(evalFile, sourceCode, "utf-8");
    const url = pathToFileURL(evalFile).href;
    ...
    try { fs.unlinkSync(evalFile); } catch {}
  }
