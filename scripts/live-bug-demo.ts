...
function printStep(step: any) {
  if (step.type === "thinking") {
    ...
    const emoji = toolEmojis[step.toolName ?? ""] ?? "🔧";
    console.log(`\n  ${emoji}  CALLING: ${step.toolName}`);
  } else if (step.type === "tool_result") {
    if (step.toolName === "read_source_code") {
      console.log(`  📥  Got source code (${readWorkerSource().split("\n").length} lines)`);
    } else {
      try {
        const parsed = JSON.parse(step.content);
        if (parsed.success === false) {
          console.log(`  📥  Tool result: ❌ ${parsed.error}`);
        } else if (parsed.changed) {
          console.log(`  📥  Code change applied:`);
          console.log(`       FROM: ${parsed.changed.from.trim()}`);
          console.log(`       TO:   ${parsed.changed.to.trim()}`);
        } else {
          const short = JSON.stringify(parsed).slice(0, 150);
          console.log(`  📥  Result: ${short}`);
        }
      } catch {
        console.log(`  📥  ${step.content.slice(0, 150)}`);
      }
    }
...
async function runScenario(bug: Bug) {
  ...
  // Step 1 — Show clean source around the target line
  ...
  if (!injected) {
    ...
    console.log("   Make sure demo/worker-logic.ts has not been manually corrupted.");
    return;
  }
  ...
  // Step 3 — Write the error log (simulates worker detecting the crash)
  ...
  // Step 5 — Route to correct agent (L3 = advisory only)
  ...
  // Step 6 — Verify the file was actually fixed
  console.log("\n" + "─".repeat(52));
  ...
  if (isFixed) {
    ...
    console.log(`     The worker will pick up logic on its next cycle.`);
  } else {
    console.log(`\n  ❌ FAILED — Bug is still present in worker-logic.ts`);
    console.log(`     The agent ran but did not edit the correct line.`);
    console.log(`     Check agent steps above for where it went wrong.`);
  }
