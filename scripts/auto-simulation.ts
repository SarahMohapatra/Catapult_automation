...
/** Automation repairs this file; worker.ts is a thin runtime shell. */
  ...
  if (step.type === "thinking") {
    ...
    try {
      ...
      if (step.toolName === "run_sandbox_tests") {
        console.log(`  📊 Test Results: ${parsed.summary}`);
        if (parsed.results) {
        ...
        if (parsed.success) {
          console.log(`  ✔  Fix staged in sandbox. Real file unchanged.`);
        } else {
          console.log(`  ✘  Could not stage fix: ${parsed.error}`);
        }
      } else if (step.toolName === "apply_sandbox_to_real") {
        if (parsed.success) {
          console.log(`  🎉 Fix applied to real file! ${parsed.testSummary}`);
        } else {
          console.log(`  ✘  Apply blocked: ${parsed.error}`);
        }
      } else if (step.toolName === "read_source_code") {
        console.log(`  📄 Source read (${parsed.totalLines} lines)`);
      } else {
...
async function processDefect(
  ...
  const baseDescription = bug
    ...
    : buildUnknownDefectDescription(validationSnapshot);
...
function startFileWatcher() {
  ...
  console.log(
    "  Catalog match (if any) is a label only, not the detector.\n"
  );
  ...
  fs.watch(WORKER_LOGIC_FILE, async (eventType) => {
    ...
    isProcessing = true;
    try {
      await processDefect(matchedBug ?? null, validation);
    } finally {
